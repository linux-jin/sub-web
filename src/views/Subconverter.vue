<template>
  <div>
          ...
          <el-container>
            <el-form :model="form"
              ...
              <el-form-item label="模式设置:">
                <el-radio v-model="advanced"
                  label="1">基础模式</el-radio>
                <el-radio v-model="advanced"
                  label="2">进阶模式</el-radio>
              </el-form-item>
              <el-form-item label="订阅链接:">
                <el-input v-model="form.sourceSubUrl"
                  type="textarea"
                  rows="3"
              ...
              <el-form-item label="客户端:">
                <el-select v-model="form.clientType"
                  style="width: 100%">
                  <el-option v-for="(v, k) in options.clientTypes"
                    :key="k"
                    :label="k"
                    :value="v"></el-option>
                </el-select>
              ...
              <div v-if="advanced === '2'">
                <el-form-item label="后端地址:">
                  <el-select v-model="form.customBackend"
                    allow-create
                    filterable
                    @change="selectChanged"
                    placeholder="可输入自己的后端"
                    style="width: 100%">
                    <el-option v-for="(v, k) in options.customBackend"
                      :key="k"
                      :label="k"
                      :value="v"></el-option>
                  </el-select>
                ...
                <el-form-item label-width="0px">
                  <el-row type="flex">
                    <el-col>
                      <el-checkbox v-model="form.nodeList"
                        label="输出为 Node List"
                        border></el-checkbox>
                    </el-col>
                    <el-popover placement="bottom"
                      ...
                      <el-row>
                        <el-checkbox v-model="form.emoji"
                          label="Emoji"></el-checkbox>
                      </el-row>
                      <el-row>
                        <el-checkbox v-model="form.scv"
                          label="跳过证书验证"></el-checkbox>
                      </el-row>
                      <el-row>
                        <el-checkbox v-model="form.udp"
                          @change="needUdp = true"
                      ...
                      <el-row>
                        <el-checkbox v-model="form.sort"
                          label="排序节点"></el-checkbox>
                      </el-row>
                      <el-row>
                        <el-checkbox v-model="form.fdn"
    ...
    <el-dialog :visible.sync="dialogUploadConfigVisible"
      :show-close="false"
      ...
      <div slot="title">
        ...
        <el-popover trigger="hover"
          ...
          <el-link type="primary"
            :href="sampleConfig"
            target="_blank"
            icon="el-icon-info">参考配置</el-link>
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
          ...
          :disabled="uploadConfig.length === 0">确 定</el-button>
      </div>
    </el-dialog>

    <el-dialog :visible.sync="dialogLoadConfigVisible"
      :show-close="false"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
      width="700px">
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
          ...
          :disabled="loadConfig.length === 0">确 定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getBackendVersion() {
      this.$axios
        .get(
          this.form.customBackend.substring(
            0,
            this.form.customBackend.length - 5
          ) + '/version'
        )
        .then(res => {
          ...
          let a =
            this.form.customBackend.indexOf('api.v1.mk') !== -1 ||
          ...
          a
            ? this.$message.success(
                `${this.backendVersion}` +
                  '肥羊负载均衡加强后端支持vless+trojan xtls订阅转换'
              )
            : b
            ? this.$message.success(
                `${this.backendVersion}` +
                  '品云实验性后端支持vless+trojan xtls订阅转换'
              )
            : c
            ? this.$message.success(
            ...
            : this.$message.success(
                `${this.backendVersion}` +
                  '官方原版后端不支持vless/trojan xtls订阅转换'
              )
        })
        .catch(() => {
          this.$message.error(
